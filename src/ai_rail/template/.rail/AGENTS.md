...
Before scoped issues exist, use `rail plan --copy` with a GitHub-connected AI agent to create a phased roadmap issue and the first small issues. After several shipped issues, use `rail phase --copy` for phase audit. Coding still happens one GitHub Issue at a time.
